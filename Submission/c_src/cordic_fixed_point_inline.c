...
static inline int32_t rotation(int32_t vector, int32_t angle){
...
static inline int32_t vectoring(int32_t vector){
	register int i, arc_tan;
	register int32_t cur_x, cur_y, int_z, rounding_mask, round_x, round_y;
...
inline int16_t arctan_fraction_inline(int16_t numerator, int16_t denominator){
...
/* angle in range [-90,90]
* First two base 10 digits are decimal*/
inline int16_t sin_inline(int32_t angle){
	int32_t vector = (1 << 14) ;
	vector = vector << 16;
	vector = rotation(vector, angle);
